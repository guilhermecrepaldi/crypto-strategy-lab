...
OWNER: `docs/microstructure/HIGH_UPTIME_RECOVERY_OWNER_DIRECTIVE.md`, superseded for capital
policy by the subsequent OWNER COMPOUNDING correction (the root preserves the exact text).
Historical parent: B10_FROZEN / RRV2_H1_B10_F0, artifact
...
Main experiment: COMPOUNDING; POSITION_SIZING=USE_AVAILABLE_OPERATING_BANK.
100USDT is initial capital ONLY. No subsequent target100, artificial reset or withdrawals.
The unregistered/unexecuted fixed-notional draft is preserved under `historical/` as
`M012_FIXED_NOTIONAL_DRAFT_PREREGISTRATION.md` and `M012_FIXED_NOTIONAL_DRAFT_SPEC.json`:
INVALID_FOR_OWNER_STRATEGY / AUXILIARY_DIAGNOSTIC_RULER, not a competing active policy.
Its spec SHA was db6fe2545d974f2195a947a99afb8152f87c7b154b426c178ca1679c064d4188;
prereg SHA was754547bf1f9812932b18d4c712c8925e1b4d0d59f960c82169c02ec5513b94e3.
M012 was not registered or executed before correction; no occupied model is overwritten.
...
A NEW entry budgets all current available operating cash C, excluding the segregated
reserve. Save that lot's initial budget before first submission. Submit its largest
filter-rounded affordable quantity, including applicable fee asset semantics. If that
quantity breaches exchange maximum quantity/notional, reject the entry explicitly rather
than clip it or create chunks; preserve all cash and flag CAPACITY_LIMIT_REACHED. Unused cash remains
operating capital, never a withdrawal. Partial entry continuation spends only the remaining
budget of THAT same lot; no second lot and no reset to100. If the rounded affordable order
fails minimum quantity/notional, block and count capital-insufficient downtime.

Required example: starting O100/R5, a realized net profit1 funds R by.05 and retains.95:
O100.95/R5.05. The next entry budgets100.95 (subject to filters), not100. Next profit again
increases the subsequent budget. After a covered forced loss, use the actually restored
current operating bank, never the campaign's initial100.

CAPACITY_LIMIT_REACHED flags binding exchange maximum quantity/notional or desired order
quantity exceeding the chosen profile's observed best-level supported size. This is a
conservative support-boundary flag, not a calibrated market-impact curve. Keep requested,
submitted and filled quantities separately, preserve residual cash/inventory, and never
cap at100. Maximum-filter entry rejection is the conservative scope boundary of this
implementation; exchange-size chunking is NOT implemented or claimed. Existing partial-fill
release retries remain the SAME lot. Finishing one IOC order does not complete a lot with
remaining operational inventory. Actual tape volume and
provided depth still constrain every fill; no scalable/size-independent fill assumption.
Any capacity flag blocks an unrestricted growth claim and opens a future capacity study;
it does not authorize changing pair, adding banks or creating fictitious liquidity.
...
(flat operating cash cannot fund the minimum valid filter-rounded order at the current
causal selected price), and flat capacity-blocked time. No double counting. Report the
capacity-blocked duration separately as well. No flat100cash threshold remains. This is readiness/lock uptime, NOT fill uptime;
...
- At every checkpoint report OPERATING_BANK, RESERVE, TOTAL_EQUITY,
  CURRENT_POSITION_NOTIONAL, CUMULATIVE_NET_PROFIT, CUMULATIVE_RESERVE_FUNDING,
  CUMULATIVE_RELEASE_LOSS, requested/submitted cycle budget and CAPACITY_LIMIT_REACHED.
  Persist equity, operating bank, reserve and cycle-notional curves; exact capital
  checkpoints DAY_1, DAY_7, DAY_30, DAY_90 and FINAL, using only the causal prefix.
  Also expose unrealized inventory/dust marks separately. No fixed100 PnL substitutes
  for the capital curve or OWNER return. FIXED_100_RESULT_NOT_OWNER_STRATEGY_RETURN=YES
  applies to the preserved B10 diagnostic, not to M012 compounding economics.
...
All gates are conjunctive for conditional acceptance and permission to propose the next
fee/capacity study; compounding is already mandatory in this primary experiment:
...
5. Realized CUMULATIVE_NET_PROFIT>0 and terminal bid-marked TOTAL_EQUITY>105 after all actual fees;
...
floor budget with adverse prices/fees; exact equity conservation; unaffordable-minimum entry block;
profit increases next budget100->100.95; no100reset;95% operating reinvestment;
partial-fill budget conservation; current-bank sizing under filters; capacity flags;
restored bank compounding; zero withdrawals; reserve transfer never counted as profit;
