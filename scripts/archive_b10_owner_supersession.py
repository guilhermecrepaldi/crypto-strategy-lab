...
from scripts.b10_checkpoint_scoreboard import digest as file_digest, read_checkpoint
...
def inspect_profile(folder: Path) -> dict[str, Any]:
    ...
    if canonical_hash(original_payload["replay"]["payload"]) != original_payload["replay"]["sha256"]:
    ...
    return {
        "profile": folder.name,
        "checkpoint": {"path": str(original), "bytes": original.stat().st_size, "sha256": original_sha},
        "owner_stop_checkpoint": {"path": str(copy), "bytes": copy.stat().st_size, "sha256": copy_sha, "matches_checkpoint_bytes_and_sha": original_sha == copy_sha},
        "journal_prefix": {"path": str(journal), "declared_prefix_bytes": declared_prefix_bytes, "declared_prefix_sha256": declared.get("sha256"), "prefix_sha256_recomputed": prefix_sha, "full_file_bytes": journal.stat().st_size if journal.exists() else None, "full_file_sha256": journal_sha, "tail_4096_sha256": tail_digest(journal) if journal.exists() else None, "raw_tail_preserved": journal.exists(), "preserved": journal.exists(), "prefix_hash_match": prefix_sha == declared.get("sha256")},
...
def publish(result):
    ...
    score = dict(b)
    score.update({"status": status, "verdict": "SUPERSEDED_PARTIAL_EXECUTION_EVIDENCE",
                  "classification": "AUXILIARY_EXECUTION_DIAGNOSTIC",
                  "FIXED_100_RESULT_NOT_OWNER_STRATEGY_RETURN": True,
                  "completed_profile_A": a, "supersession_manifest": "reports/usdcusdt/B10-owner-supersession.json",
                  "audit_status": "HASH_BOUND_PRESERVATION_PASS; economic/raw independent audit earlier A prefix only"})
    Path("reports/usdcusdt/B10-reality-scoreboard.json").write_text(json.dumps(score,indent=2)+"\n",encoding="utf-8")
    keys = ("simulation_timestamp", "progress_percent", "full_fill_cycles", "net_positive_cycles",
            "net_pnl_fixed_100", "reserve_final", "release_filled", "zero_cycle_days_so_far", "max_hold", "lock_hours")
    report = '# B10 REALITY — CURRENT SCOREBOARD\n\nSTATUS='+status+'\n\nFIXED_100_RESULT_NOT_OWNER_STRATEGY_RETURN=YES. Valores abaixo são diagnósticos auxiliares; não retorno da estratégia composta.\n\n'
    report += '| Métrica | A concluído antes da ordem | B último checkpoint parcial |\n|---|---|---|\n'
    ...
    fields = {"LAST_B10_RUN_ID":b["run_id"],"LAST_PROFILE":b["profile_name"],
              "LAST_SIMULATION_TIMESTAMP":b["simulation_timestamp"],"LAST_PROGRESS":b["progress_percent"],
              "LAST_FULL_FILL_CYCLES":b["full_fill_cycles"],"LAST_NET_POSITIVE_CYCLES":b["net_positive_cycles"],
              "LAST_NET_PNL_FIXED_100":b["net_pnl_fixed_100"],"LAST_RESERVE":b["reserve_final"],
              "LAST_RELEASES":b["release_filled"],"LAST_ZERO_DAYS":b["zero_cycle_days_so_far"],
              "LAST_MAX_HOLD":b["max_hold"],"LAST_LOCK_HOURS":b["lock_hours"]}
...
def main() -> int:
    ...
    result = {
        ...
        "status": "SUPERSEDED_BY_OWNER_STRATEGY_UPDATE",
        "reason": "Reality evidence demonstrated unacceptable capital lock / zero-cycle behavior under the current B10 policy.",
        "process_confirmation_external": args.process_confirmed,
        "process_identity": {"run_id": args.runs.name, "writer_confirmed_stopped_by_owner": args.process_confirmed},
